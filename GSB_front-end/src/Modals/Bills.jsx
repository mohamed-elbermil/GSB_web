...
export function BillsList({ userRole = "user" }) {
  ...
  const handleDeleteBill = async (billId) => {
    if (window.confirm("Êtes-vous sûr de vouloir supprimer cette facture ?")) {
      try {
        ...
        setBills((prevBills) =>
          prevBills.filter((bill) => (bill._id || bill.id) !== billId),
        );
  ...
  const handleUpdateBill = async (updatedBill) => {
    try {
      ...
      setBills((prevBills) =>
        prevBills.map((bill) =>
          (bill._id || bill.id) === (updatedBill._id || updatedBill.id)
            ? updatedBill
            : bill,
        ),
      );
  ...
  return (
    <div className="bills-container">
      <div className="bills-header">
        ...
        {userRole !== "admin" && (
          <button className="add-bill-button" onClick={openAddModal}>
            ...
            <tbody>
              {bills.map((bill) => (
                <tr key={bill._id || bill.id} className="bill-row">
                  ...
                  <td>
                    <span
                      className={`bill-status status-${bill.status.toLowerCase()}`}
                    >
                      {bill.status}
                    </span>
                  </td>
      ...
      <AddBillModal isOpen={isAddModalOpen} onClose={closeAddModal} />
  ...
  return (
    ...
    >
      <div className="modal-wrapper">
        ...
        <span className="modal-alignment-helper" aria-hidden="true">
          &#8203;
        </span>
            ...
            <div className="modal-header">
              <div className="modal-title-container">
                <div className="modal-title-wrapper">
                  ...
                  <span
                    className={`status-badge ${getStatusClasses(bill.status)}`}
                  >
                    {bill.status}
                  </span>
                </div>
                      ...
                      <div className="proof-link">
                        {(bill.proof || bill.Proof).startsWith("http") ? (
                          <div className="proof-container">
                            <img
                              src={`${API_URL}/images/proxy?url=${encodeURIComponent(bill.proof || bill.Proof)}`}
                              alt="Proof document"
                              ...
                              onError={(e) => {
                                ...
                                fallback.className = "proof-fallback";
                                fallback.innerHTML = `
                                  <div class="proof-info">
                                    <svg class="proof-icon" fill="currentColor" viewBox="0 0 20 20">
                                      <path fillRule="evenodd" d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z" clipRule="evenodd" />
                                    </svg>
                                    <span class="proof-text">Image non disponible (accès admin requis)</span>
                                  </div>
                                `;
                            ...
                            <div className="proof-actions">
                              <a
                                href={`${API_URL}/images/proxy?url=${encodeURIComponent(bill.proof || bill.Proof)}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="proof-link-text"
                                onClick={(e) => {
                                  // Ajouter le token d'authentification à l'URL pour le nouvel onglet
                                  const token =
                                    localStorage.getItem("token") ||
                                    sessionStorage.getItem("token");
                                  if (token) {
                                    const newUrl = `${API_URL}/images/proxy?url=${encodeURIComponent(bill.proof || bill.Proof)}&token=${token}`;
